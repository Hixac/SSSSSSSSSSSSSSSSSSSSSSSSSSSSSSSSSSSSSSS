...
class ManysError(Exception):
    """
    Base exception class for all errors raised by Manys.
...
class ManysTaskError(ManysError):
    """
...
class ManysRedirectionError(ManysError):
    """
...
class InternalServerError(ManysError):
    def __init__(
...
class ResourceNotFound(ManysError):
...
class ResourceUnavailable(ManysError):
    def __init__(self, message: str = "Unavailable", status_code: int = 410) -> None:
        super().__init__(message, status_code)


class ResourceAlreadyExists(ManysError):
